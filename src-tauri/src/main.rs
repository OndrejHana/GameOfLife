...
impl Hra {
  fn new (rozliseni: u32) -> Hra {
    ...
    Hra {
      rozliseni,
      mapa: (0..(rozliseni * rozliseni)).map(|_| {
  ...
  fn stav(&self, mut x: i32, mut y: i32) -> bool {
    ...
    while x < 0 { x += rozliseni }
    while x >= rozliseni { x -= rozliseni; }
    while y < 0 { y += rozliseni; }
    while y >= rozliseni { y -= rozliseni; } 
  ...
  fn dalsi_generace(&mut self) {
    for x in 0..self.rozliseni {
      for y in 0..self.rozliseni {
        ...
        self.mapa_dalsi.insert(index_policka, policko_dalsi_generaci);
      }
    }
  }
